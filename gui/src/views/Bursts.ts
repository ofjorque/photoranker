...
export async function renderBursts(container: HTMLElement): Promise<void> {
  ...
  if (!project) {
    container.innerHTML = '<div class="view"><div class="empty-state">Abrí un proyecto primero.</div></div>';
    return;
  }
  ...
  mountRankingBoard(boardContainer, {
    dbPath: project.dbPath,
    images: burst.images,
    onSubmit: async (ranking) => {
      try {
        const result = await cli.burstTournament(project.dbPath, burst.id, ranking);
        showToast(`Burst #${burst.id}: ganadora ${result.representative_image_id}, ${result.rejected} rechazadas`);
        renderBursts(container);
      } catch (e) {
